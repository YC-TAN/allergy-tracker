...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            ...
            404,
            id="valid_format_no_entry",
        ),
        pytest.param("invalid-date", 422, id="malformed_string"),
        pytest.param("9999-99-99", 422, id="invalid_month_day"),
    ],
)
def test_get_entry_invalid_or_not_exist_returns_404_or_422(
    client, invalid_date, expected_status
):
...
def test_upsert_entry_when_exist_success(client: TestClient, session: Session):
    ...
    response = client.put(f"/api/entries/{FIXED_TODAY.isoformat()}", json=payload)
    ...
    assert (
        normalize_timestamp(data["created_at"]) == initial_created_at.isoformat()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            {**valid_payload, "symptoms": ["eyes", "elbow"]},
            id="known_and_unknown_symptoms",
        ),
        pytest.param(
            {**valid_payload, "notes": "a" * 256}, id="notes_greater_than_255"
        ),
    ],
)
...
def test_get_entries_default_days_returns_last_7_days(client, session):
    ...
    create_existing_entry(
        session, (FIXED_TODAY - timedelta(days=7)).isoformat()
    )  # out of range
...
def test_get_entries_custom_days(client, session):
    ...
    create_existing_entry(
        session, (FIXED_TODAY - timedelta(days=30)).isoformat()
    )  # out of range
