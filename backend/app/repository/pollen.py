...
def get_allergen_data(check_date: date, location: str, session: SessionDep) -> PollenForecast | None:
...
def create_allergen_data(allergen_data: dict, session: SessionDep) -> PollenForecast:
    ...
    return session.exec(statement).scalars().one_or_none()
    # db.commit()
